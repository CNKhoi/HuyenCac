...
function applyHeaderScroll(){const topbar=$(".topbar");if(!topbar)return;topbar.classList.toggle("scrolled", window.scrollY>10)}
function attachRippleEffect(selector){$$(selector).forEach(el=>{if(el.dataset.rippleBound)return;el.dataset.rippleBound="1";el.addEventListener("click",e=>{const rect=el.getBoundingClientRect();const ripple=document.createElement("span");ripple.className="ripple";const size=Math.max(rect.width,rect.height)*1.4;ripple.style.width=size+"px";ripple.style.height=size+"px";ripple.style.left=(e.clientX-rect.left)+"px";ripple.style.top=(e.clientY-rect.top)+"px";el.appendChild(ripple);setTimeout(()=>ripple.remove(),700)})})}
function initGlobalUIEffects(){applyHeaderScroll();attachRippleEffect(".btn, .mini-btn, .nav-link");window.addEventListener("scroll",applyHeaderScroll,{passive:true})}
function initTarotHoverEffects(){const cards=$$(".tarot-reading");cards.forEach(card=>{if(card.dataset.hoverBound)return;card.dataset.hoverBound="1";const face=$(".tarot-card-face",card);if(!face)return;card.addEventListener("mousemove",e=>{const rect=card.getBoundingClientRect();const px=(e.clientX-rect.left)/rect.width;const py=(e.clientY-rect.top)/rect.height;const ry=(px-.5)*8;const rx=(.5-py)*7;face.style.setProperty("--mx",`${Math.round(px*100)}%`);face.style.setProperty("--my",`${Math.round(py*100)}%`);face.style.transform=`rotateX(${rx}deg) rotateY(${ry}deg)`});card.addEventListener("mouseleave",()=>{face.style.transform="";face.style.removeProperty("--mx");face.style.removeProperty("--my")})})}
...
staggerCards("#tarotGrid .tarot-reading");playTarotDealAnimation();initTarotHoverEffects();startMotion($("#view-tarot"));
}

function playTarotDealAnimation(){const grid=$("#tarotGrid");if(!grid)return;const cards=$$(".tarot-reading",grid);grid.classList.add("is-dealing");cards.forEach((card,i)=>{card.classList.remove("dealt");setTimeout(()=>card.classList.add("dealt"),80 + i*110)});setTimeout(()=>grid.classList.remove("is-dealing"),900 + cards.length*80)}
...
function scanDates(){
 ...
 $("#bestDate").textContent=fmtVN(best.date,{day:"2-digit",month:"2-digit"});$("#bestDateMeta").textContent=`${fmtVN(best.date,{weekday:"short",day:"2-digit",month:"2-digit",year:"numeric"})} • ${best.dc.stem} ${best.dc.branch}`;$("#bestScore").textContent=best.score;$("#goodDateCount").textContent=good;$("#dateProfileBranch").textContent=`${by.stem} ${by.branch}`;$("#dateProfileElement").textContent=`Tuổi ${by.animal} • ${by.element}`;$("#dateRankingTitle").textContent=`${PURPOSE_NAME[p]} — xếp từ phù hợp cao xuống thấp`;renderDateList();animateNumber($("#bestScore"),best.score);animateNumber($("#goodDateCount"),good)
}
function renderDateList(){const list=showAllDates?dateCache:dateCache.slice(0,10);$("#toggleAllDates").classList.toggle("hidden",dateCache.length<=10);$("#toggleAllDates").textContent=showAllDates?"Thu gọn":"Xem tất cả";$("#dateResults").innerHTML=list.map((x,index)=>{const [label,cls]=rankLabel(x.score);return `<article class="date-item"><div class="date-main"><div class="date-cal"><small>${fmtVN(x.date,{month:"short"})}</small><strong>${String(x.date.getDate()).padStart(2,"0")}</strong><small>${fmtVN(x.date,{weekday:"short"})}</small></div><div class="date-info"><strong>${index===0?"⭐ ":""}${x.dc.stem} ${x.dc.branch} • Âm ${x.lunar.day}/${x.lunar.month}${x.lunar.leap?"N":""}</strong><p>${label}. ${index===0?"Đây là ngày đứng đầu trong khoảng đã chọn.":"Bấm bên dưới nếu muốn xem lý do cộng/trừ điểm."}</p></div><div class="score-badge ${cls}"><strong>${x.score}</strong><small>${label}</small></div></div><details class="factor-details"><summary>Vì sao ngày này được ${x.score} điểm?</summary><div class="score-breakdown">${x.factors.map(f=>`<div class="factor ${f.score>0?"plus":f.score<0?"minus":"zero"}"><span><b>${f.label}</b><br>${f.detail}</span><b>${f.score>0?"+":""}${f.score}</b></div>`).join("")}</div></details></article>`}).join("");staggerCards("#dateResults .date-item");startMotion($("#view-dates"))}

function animateNumber(el,to,duration=650){if(!el||!Number.isFinite(to))return;const start=Number(el.dataset.animFrom||0),startTime=performance.now();const tick=(now)=>{const p=Math.min(1,(now-startTime)/duration),val=Math.round(start+(to-start)*(1-Math.pow(1-p,3)));el.textContent=val;if(p<1)requestAnimationFrame(tick);else el.dataset.animFrom=String(to)};requestAnimationFrame(tick)}
const revealObserver = typeof IntersectionObserver!=="undefined" ? new IntersectionObserver(entries=>{entries.forEach(entry=>{if(entry.isIntersecting){entry.target.classList.add("is-visible");revealObserver.unobserve(entry.target)}})},{threshold:.12,rootMargin:"0px 0px -40px 0px"}) : null;
function startMotion(scope=document){const targets=$$(".welcome-card,.choice-card,.simple-how,.logic-strip,.page-title,.guide-card,.result-panel,.analysis-panel,.primary-reading-card,.padded,.advanced,.best-date-panel,.date-ranking-panel,.profile-banner,.pillar-card,.relation-item,.gender-kua,.cycle-row,.metric-card",scope);targets.forEach((el,i)=>{if(!el.classList.contains("reveal"))el.classList.add("reveal");el.style.transitionDelay=`${Math.min(i,8)*45}ms`;if(revealObserver){revealObserver.observe(el)}else{el.classList.add("is-visible")}})}
function staggerCards(selector){$$(selector).forEach((el,i)=>{el.classList.add("reveal");el.style.transitionDelay=`${i*70}ms`;requestAnimationFrame(()=>el.classList.add("is-visible"))})}
...
function switchView(name){document.body.dataset.view=name;$$(".view").forEach(v=>v.classList.toggle("active",v.id===`view-${name}`));$$(".nav-link").forEach(b=>b.classList.toggle("active",b.dataset.view===name));window.scrollTo({top:0,behavior:"smooth"});requestAnimationFrame(()=>startMotion($(`#view-${name}`)))}
function renderAll(){const current=$$(".view").find(v=>v.classList.contains("active"))?.id?.replace("view-","")||"home";document.body.dataset.view=current;renderHeader();renderNumerology();renderHoroscope();if(profile){const by=yearCanChi(new Date(profile.birthDate+"T12:00:00").getFullYear());$("#dateProfileBranch").textContent=`${by.stem} ${by.branch}`;$("#dateProfileElement").textContent=`Tuổi ${by.animal} • ${by.element}`;animateNumber($("#homeLife"), Number($("#homeLife").textContent)||0)}requestAnimationFrame(()=>startMotion($(".view.active")))}
