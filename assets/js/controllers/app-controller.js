...
export const AppController={
  ...
  bindDelegatedEvents(){
    document.addEventListener('click',e=>{const target=e.target.closest('[data-action]');if(!target){if(e.target.id==='profileModal')UIManager.closeProfile();return}const action=target.dataset.action;
      ...
      else if(action==='profile-clear'){StorageModel.clear();AppState.patch({profile:null,tarot:null,dates:null});UIManager.closeProfile()}
  ...
  requireProfile(){const p=AppState.get().profile;if(!p){UIManager.openProfile(null);return null}return p},
  drawTarot(){const profile=this.requireProfile();if(!profile)return;try{const opts={mode:UIManager.q('#tarotMode').value,topic:UIManager.q('#tarotTopic').value,question:UIManager.q('#tarotQuestion').value,presetQuestion:UIManager.q('#tarotPresetQuestion').value};AppState.patch({tarot:TarotEngine.draw(profile,opts,new Date())})}catch(err){UIManager.toast(err.message)}},
  scanDates(){const profile=this.requireProfile();if(!profile)return;const fv=UIManager.q('#dateFrom').value,tv=UIManager.q('#dateTo').value,purpose=UIManager.q('#datePurpose').value;if(!fv||!tv){UIManager.toast('Vui lòng chọn đủ từ ngày và đến ngày.');return}try{const data=DateScorer.range(profile,purpose,new Date(fv+'T12:00:00'),new Date(tv+'T12:00:00'));AppState.patch({dates:data,showAllDates:false})}catch(err){UIManager.toast(err.message)}}
};
